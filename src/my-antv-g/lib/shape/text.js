...
var Text = /** @class */ (function (_super) {
    ...
    Text.prototype._assembleFont = function () {
        ...
        attrs.font = (0, g_base_1.assembleFont)(attrs);
    };
    // 如果文本换行，则缓存数组
    Text.prototype._setText = function (text) {
        var textArr = null;
        if ((0, util_1.isString)(text) && text.indexOf('\n') !== -1) {
            textArr = text.split('\n');
        }
    ...
    Text.prototype._drawTextArr = function (context, textArr, isFill) {
        ...
        var height = (0, g_base_1.getTextHeight)(attrs.text, attrs.fontSize, attrs.lineHeight);
        var subY;
        (0, util_1.each)(textArr, function (subText, index) {
            subY = y + index * (spaceingY + fontSize) - height + fontSize; // bottom;
            if (textBaseline === 'middle')
                subY += height - fontSize - (height - fontSize) / 2;
            if (textBaseline === 'top')
                subY += height - fontSize;
            if (!(0, util_1.isNil)(subText)) {
                if (isFill) {
