...
function applyAttrsToContext(context, element) {
    ...
    for (var k in attrs) {
        ...
        if (name_1 === 'matrix' && v) {
            ...
            (0, util_1.isArray)(v) && context.setLineDash(v);
        }
        else {
            if (name_1 === 'strokeStyle' || name_1 === 'fillStyle') {
                // 如果存在渐变、pattern 这个开销有些大
                // 可以考虑缓存机制，通过 hasUpdate 来避免一些运算
                v = (0, parse_1.parseStyle)(context, element, v);
            }
...
function checkRefresh(canvas, children, region) {
    ...
    (0, util_1.each)(refreshElements, function (el) {
        if (el !== canvas) {
...
function checkElementRefresh(shape, region) {
    ...
    var isAllow = shape.cfg.isInView && bbox && (0, util_2.intersectRect)(bbox, region);
    return isAllow;
}
// 绘制 path
        ...
        switch (command) {
            ...
            case 'A': {
                ...
                if (arcParamsCache) {
                    ...
                    if (!arcParams) {
                        arcParams = (0, arc_params_1.default)(currentPoint, params);
...
function getRefreshRegion(element) {
    ...
    if (!element.destroyed) {
        ...
        if (validCache && validBBox) {
            region = (0, util_2.mergeRegion)(cacheBox, bbox);
        }
        else if (validCache) {
            region = cacheBox;
        }
...
function getMergedRegion(elements) {
    ...
    (0, util_1.each)(elements, function (el) {
        ...
        var minX = region.minX, minY = region.minY, maxX = region.maxX, maxY = region.maxY;
        if (minX < mergedMinX) {
            mergedMinX = minX;
        }
        if (minY < mergedMinY) {
...
function mergeView(region, viewRegion) {
    ...
    // 不相交，则直接返回 null
    if (!(0, util_2.intersectRect)(region, viewRegion)) {
        return null;
    }
    return {
